...
{
    public class Survey_DelPage_Layer
    {
        ...
        public int UpdatePageTable(string PageNo, string UID, string SID)
        {
            ...
            //StringBuilder sql = new StringBuilder("UPDATE" + TabelName + "SET PageNo=PageNo-1 WHERE PageNo>@PageNo AND UID=@UID AND SID=@SID", 50);
            StringBuilder sql = new StringBuilder("UPDATE" + TabelName + "SET PageNo=PageNo-1 WHERE PageNo>@PageNo AND "+initRightSql+" AND SID=@SID", 50);
        ...
        public int DeletePageTable(string SID, string UID, string PID)
        {
            initRightSql = new InitRight().initSurveyRight("UID=@UID ", SID, " ");
            string TabelName = " " + prefix + "PageTable ";
            StringBuilder sql = new StringBuilder("DELETE" + TabelName + "WHERE SID=@SID AND "+initRightSql+" AND PID=@PID", 50);
            SqlParameter[] parameters = new SqlParameter[3];
        ...
        public int UpdateItemTable(string PageNo, string UID, string SID)
        {
            initRightSql = new InitRight().initSurveyRight("UID=@UID ", SID, " ");
        ...
        public SqlDataReader GetPageTable(string SID, string UID, string PID)
        {
            initRightSql = new InitRight().initSurveyRight("UID=@UID ", SID, " ");
            string TabelName = " " + prefix + "PageTable ";
            StringBuilder sql = new StringBuilder("SELECT TOP 1 PageNo FROM" + TabelName + "WHERE SID=@SID AND "+initRightSql+" AND PID=@PID", 50);
            SqlParameter[] parameters = new SqlParameter[3];
            parameters[0] = new SqlParameter("@SID", SID);
            parameters[1] = new SqlParameter("@UID", UID);
            parameters[2] = new SqlParameter("@PID", PID);
