...
namespace BusinessLayer.Survey
{
    public class Survey_PS_Layer
    {
        ...
        public DataTable GetSurveyTable1(string UID, string SID)
        {
            ...
            string TabelName = " " + prefix + "SurveyTable ";
            //StringBuilder sql = new StringBuilder("SELECT TOP 1 SurveyName,TempPage,State,Active,SID,Par,Lan FROM" + TabelName + "WHERE UID=@UID AND SID=@SID", 50);
            StringBuilder sql = new StringBuilder("SELECT TOP 1 SurveyName,TempPage,State,Active,SID,Par,Lan FROM" + TabelName + "WHERE "+initRightSql+" AND SID=@SID", 50);
