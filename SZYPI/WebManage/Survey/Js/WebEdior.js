...
function OQSSEditorFF(sEditorID,sColorPancel,sMaskID,sInstanceName,sWebSource,blnEnEditCode){
	...
	this._initEditor = function(){
		...
		$("FileManageBox").innerHTML = "<div class='TableHead' id=CloseWinBT title='关闭窗口'><div style='line-height:25px;margin-left:10px;font-weight:bold'>插入媒体</div><div class='oqss-close'></div></div><iframe hspace=0 scrolling=no src='' style='width:100%;height:375px' frameborder=0 marginheight=0 marginwidth=0 id='FileManageWin'></iframe>";
		...
		for(i=0;i<arrTools[0].length;i++){
			switch(arrTools[0][i]){
				...
				case "fontfamily":
					...
					sToolBar += "<div id=BT"+i+" style='float:left;margin-left:2px' class='oqss-"+arrTools[0][i]+"'><select id='fontfamily' style='width:85px;height:19px'>"+sTemp+"</select></div>";
					sTemp = "";
					break;
				case "fontsize":
					...
					sToolBar += "<div id=BT"+i+" style='float:left;margin-left:2px'  class='oqss-"+arrTools[0][i]+"'><select id='fontsize'  style='width:50px;height:19px;overflow:hidden'>"+sTemp+"</select></div>";
					sTemp = "";
					break;
				default:
					sToolBar += "<div  onmouseover='this.className=\"BT_Over\"'   onmouseout='this.className=\"BT_Normal\"'   class='BT_Normal'><div id='BT"+i+"' style='width:16px;height:16px'  class='oqss-"+arrTools[0][i]+"' title='"+arrTools[1][i]+"'></div></div>";	
				break;
			}
		}
	 ...
	 this.initocolor = function(){
		...
		colorTable='<table  border="0" cellspacing="0" cellpadding="0" style="border:1px #000 solid;width:253px;" id="colortable">'
					+'<tr style="height:30px"><td colspan=21 style="background-color:#EEE">'
			   		...
			   		+'<tr><td width="3"><td><input type="text" name="DisColor" id="DisColor" size="6" disabled style="border:solid 1px #000;background-color:#FF0"><div class="oqss-close" onclick=document.getElementById("'+sColorPancel+'").style.display="none"></div></td>'
