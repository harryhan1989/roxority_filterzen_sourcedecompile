...
//选择日期 → 由  2007-06-10 添加，通过 ID 来选日期
...
function SelectDate(obj, strFormat, x, y) {
    ...
    topY = (y == null) ? topY : y; //自定义定位偏移量 2007-02-11 
...
Calendar.prototype.draw = function() {
    ...
    obj.onclick = function() {
        calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*///2007-09-14 
    }
...
this.innerHTML).format(calendar.dateFormatStyle));
            ...
            tds[i].style.cursor = "pointer"; //2007-08-06 添加，鼠标变成手指状
...
Calendar.prototype.show = function(dateObj, popControl) {
    ...
    var now = new Date();
    this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle); //2008-01-29  → 若为空则根据dateFormatStyle初始化日期
    ...
    this.panel.style.left = (xy.x + leftX) + "px"; // 2007-02-11 修改 → 加入自定义偏移量
    this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";
    // 2006-06-25 修改 → 把 visibility 变为 display，并添加失去焦点的事件 //this.setDisplayStyle("select", "hidden");
...
// 2007-07-26 修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方
function InitContainerPanel() //初始化容器
{
    var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #fff;border: 1px solid #CCCCCC;width:175px;font-size:12px;"></div>';
    if (document.all) {
