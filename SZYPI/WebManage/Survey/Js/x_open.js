...
var win_frame = "<div  id='x_open_win' name='x_open_win' style='position:absolute;z-index:100; width: 420px; height: 350px ;left:10px;top:10px;font-size:12px; display:none ' onselectstart='return false'>\r\n";
win_frame += "<div>\r\n";
...
win_frame += "<img src='" + close_img + "' id='close_button_name'  width='16' height='21' border='0' title='关闭窗口' />";
win_frame += "</td>\r\n";
win_frame += "	</tr>\r\n";
win_frame += "</table>\r\n";
win_frame += "</div>\r\n";
...
win_frame += "		<td>\r\n";
win_frame += "		<iframe id='x_open_frame' name='x_open_frame' src='" + loading_page + "' frameborder=0 noresize style='width:100%; height:100%;background-color:overflow:auto; #ffffff;color: #333;margin: 0px; padding: 0px;border:0px '></iframe>\r\n";
win_frame += "		</td>\r\n";
...
// obj
var x_open_win_id;
var x_open_content_id;
var title_msg_layer_id;
var x_open_frame_id;
var max_button_name_id;
var close_button_name_id;
var border_img_name1_id;
var border_img_name2_id;
var x_open_win_border_id;
var size_info_layer_id;
...
function initparam() {
    ...
    x_open_win_id = document.getElementById("x_open_win");
    x_open_content_id = document.getElementById("x_open_content");
    title_msg_layer_id = document.getElementById("title_msg_layer");
    x_open_frame_id = document.getElementById("x_open_frame");
    max_button_name_id = document.getElementById("max_button_name");
    close_button_name_id = document.getElementById("close_button_name");
    border_img_name1_id = document.getElementById("border_img_name1");
    border_img_name2_id = document.getElementById("border_img_name2");
    x_open_win_border_id = document.getElementById("x_open_win_border");
    size_info_layer_id = document.getElementById("size_info_layer");
    ...
    dragresized = false;
    minrestore = 0;
    x_open_ie5 = document.all && document.getElementById;
    x_open_ns6 = document.getElementById && !document.all;
    title_height = 36;


    centerL = document.body.clientWidth / 2;
    centerT = document.body.clientHeight / 2;
    toprecord = 0;
    leftrecord = 0;
}

function x_open(title, url, width, height, otherproperties) {
    //初始化变量
    initparam();
    //    close_button_name_id.click = "closeit();refresh();";
    close_button_name_id.onclick = Function("closeit();refresh();");  //默认关闭事件
    //otherproperties  其他属性设置,事件名+冒号+美元符；  closefuncation:closeit();refresh();$maxfuncation:;
    //属性名： closefuncation --关闭按钮事件；
    //         maxfuncation   --最大化事件；

    //设置closefuncation事件

    try {
        var properties = otherproperties.toString();

        if (properties.indexOf("closefuncation") >= 0) {
            var closefuncation = properties.match("[^closefuncation].*[$]")[0].replace("$", "").replace(":", "");
            close_button_name_id.onclick = Function(closefuncation);
        }
    }
    catch (e) {
    ...
    if (!x_open_ie5 && !x_open_ns6)
        ...
        centerL = document.body.clientWidth / 2;
        centerT = document.body.clientHeight / 2;
        x_open_win_id.style.top = centerT - height / 2 + "px";
        x_open_win_id.style.left = centerL - width / 2 + "px";
...
function maximize() {
    if (minrestore == 0) {
        ...
        toprecord = x_open_win_id.style.top
        leftrecord = x_open_win_id.style.left
        ...
        centerL = document.body.clientWidth / 2;
        centerT = document.body.clientHeight / 2;
        x_open_win_id.style.top = 0 + "px";
        x_open_win_id.style.left = 8 + "px";
        ...
        change_size(initialwidth, initialheight);
        x_open_win_id.style.top = toprecord;
        x_open_win_id.style.left = leftrecord;
    }
...
function change_size(w, h) {
    ...
    if (h > 0) {
        border_img_name2_id.style.height = x_open_frame_id.style.height = h + "px";
        border_img_name1_id.style.height = border_img_name2_id.style.height;
        x_open_win_id.style.height = border_img_name1_id.style.height;
        ...
        border_img_name2_id.style.height = x_open_frame_id.style.height = 0 + "px";
        border_img_name1_id.style.height = border_img_name2_id.style.height;
        x_open_win_id.style.height = border_img_name1_id.style.height;
