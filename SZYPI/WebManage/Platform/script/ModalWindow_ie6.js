...
{
	setSelectControlStatus(wid,true,isTop);
	
	var d = getDocument(isTop);
	
	var w1 = d.getElementById("xMsg"+wid);
	var w2 = d.getElementById("xMsg"+wid+"bg");
...
{
    return "<iframe style=' visibility:inherit; width:100%; height:100%;' src='" +url+"' frameborder='no'></iframe>";
}

//创建提示框标签
function createAlertTag(msg,wid)
{
    return "<table style='width:100%;height:100%;' border='0' cellspacing='0' cellpadding='6'><tr><td align='center' valign='middle' style='height:60%'>"+msg+"</td></tr><tr><td align='center' valign='buttom'><input type='button' name='Button' class='Button' style='width:85px;' onclick='closeModalWindow(\""+wid+"\",false);' value='确定'></td></tr><tr><td style='height:10px'></td></tr></table>"
}

//创建确认框标签
function createConfirmTag(msg,wid,controlID) 
{
    return "<table style='width:100%;height:100%;' border='0' cellspacing='0' cellpadding='6'><tr><td align='center' valign='middle' colspan='2' style='height:60%'>"+msg+"</td></tr><tr><td align='right'><input type='button' name='Button' class='Button' style='width:85px;' onclick='confirmCloseModalWindow(\""+wid+"\",true);' value='确定'></td><td align='left'><input type='button' name='Cancel' class='Button' style='width:85px;' onclick='confirmCloseModalWindow(\""+wid+"\",false);' value='取消'></td></tr><tr><td style='height:10px'></td></tr></table>"
}

//创建确认框标签
function createToolBarConfirmTag(msg,wid,toolbarID,buttonKey)
{
    return "<table style='width:100%;height:100%;' border='0' cellspacing='0' cellpadding='6'><tr><td align='center' valign='middle' colspan='2' style='height:60%'>"+msg+"</td></tr><tr><td align='right'><input type='button' name='Button' class='Button' style='width:85px;' onclick='confirmToolbarCloseModalWindow(\""+wid+"\",true,\""+toolbarID+"\",\""+buttonKey+"\");' value='确定'></td><td align='left'><input type='button' name='Cancel' class='Button' style='width:85px;' onclick='confirmToolbarCloseModalWindow(\""+wid+"\",false,null,null);' value='取消'></td></tr><tr><td style='height:10px'></td></tr></table>"
}

//--------------------------------------------------------------设置下拉菜单状态--------------------------------------------------------------
//设置下拉菜单状态
function setSelectControlStatus(owner,status,isTop)
{
    var tdoc,doc,idoc;
    tdoc = getDocument(isTop);//第一级
    setSelectControlStatus_document(owner,status,tdoc);//调用锁定

    if (tdoc.frames.length != 0)
    {
        for (i=0;i<tdoc.frames.length;i++)
        {
            doc = tdoc.frames[i].self.window.document;//第二级
            if (doc != null)
            {
                setSelectControlStatus_document(owner,status,doc);//调用锁定
            }  
            
            if (tdoc.frames[i].frames.length != 0)
            {
                for (j=0;j<tdoc.frames[i].frames.length;j++)
                {
                    idoc = tdoc.frames[i].frames[j].self.window.document;//第三级
                    if (idoc != null)
                    {
                        setSelectControlStatus_document(owner,status,idoc);//调用锁定
                    }
                } 
            } 
        }
...
//设置下拉菜单状态
function setSelectControlStatus_document(owner,status,doc)
{
    if (doc.forms.length == 0) return;
    
    var o = doc.getElementById("_lockDropdownlist");//获得存储值对象
    var arrSelect = o == null ? "" : o.value;
    
    var _owner = "";
    if (arrSelect.indexOf("|") > -1)
    {
        _owner = arrSelect.split("|")[0];
        //alert(_owner + "~~~" + owner);
        if (_owner != owner) return;//已经有别的对象锁定过本页
    }
    if (!status) arrSelect = "";

    for(var i = 0;i < doc.forms[0].elements.length;i++)
    {
        e = doc.forms[0].elements[i];
        if (e.type == "select-one")
        {
            
            if (status)//解锁由本程序锁定的控件
            {
                var _arrSelect = arrSelect;
                if (arrSelect.indexOf("|") > -1) _arrSelect = arrSelect.split("|")[1];
                if (_arrSelect.indexOf(e.id) < 0) 
                {
//                    e.disabled = false;
//                    alert(e.style.display + "1");
                    e.style.display = "";
                }
                if(e.name = "pager_select")
                {
//                    e.disabled = false;
//                    alert(e.style.display + "2");
            ...
            {
//                if (e.disabled || e.disabled == "disabled") 
                  if (e.style.display == "none")
                  {
                        arrSelect += e.id + ",";//记录下原来就已锁定的控件ID
                  }
//                    e.disabled = true;
//                   alert(e.style.display + "3");
    ...
    if (!status)//锁定
    {
        if (arrSelect.substring(arrSelect.length-1,1) == ",") arrSelect = arrSelect.substring(0,arrSelect.lenth);
        arrSelect = owner + "|" + arrSelect;//加入所有者标记
        if (o != null) o.value = arrSelect;
    }
}
...
{    
	...
	setSelectControlStatus(wid,false,true);
...
function showAlert(title,width,height,msg)
{
	...
	setSelectControlStatus(wid,false,false);
...
{
    showToolbarConfirm("系统提示",210,120,msg,oToolbar.Id,oButton.Key);
