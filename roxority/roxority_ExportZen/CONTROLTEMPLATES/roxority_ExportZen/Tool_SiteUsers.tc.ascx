...
<ul>
	...
	%>
	<li><a href="/_layouts/userdisp.aspx?Force=1&ID=<%= kvp.Value.ID%>&Source=<%= Server.UrlEncode (Request.Url.ToString ())%>"><%= Server.HtmlEncode (kvp.Value.LoginName)%> &mdash; <%= Server.HtmlEncode (kvp.Value.Name)%></a></li>
	<%
		}
	%>
</ul>