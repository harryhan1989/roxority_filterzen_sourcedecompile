...
<script runat="server">
	...
	private SPFarm spFarm;
	private Hashtable spProps;
	...
	public SPFarm SpFarm {
		get {
			if (spFarm == null)
				spFarm = ProductPage.GetFarm (SpContext);
			return spFarm;
		}
	}

	public Hashtable SpProps {
		get {
			if (spProps == null)
				spProps = SpFarm.Properties;
			return spProps;
		}
	}
...
<%
	...
	Converter<KeyValuePair<string, bool>, JsonSchemaManager> getSchemaMan = delegate (KeyValuePair<string, bool> what) {
		...
		if (!fjss.ContainsKey (what.Key)) {
			kvp = JsonSchemaManager.TryGet (ProdPage, Context.Server.MapPath ("/_layouts/" + ProductPage.AssemblyName + "/" + what.Key), true, true, what.Key.EndsWith ("schemas.tl.json") ? "roxority_Shared" : null);
	...
	<%
		...
		<%
			if (!ProductPage.LicEdition (SPContext.Current, (IDictionary) null, ProductPage.IsWhiteLabel ? 0 : 4))
				...
				ProductPage.Loc (SpContext, SpFarm, SpProps, e2.Key + string.Empty, null, null, e2.Value + string.Empty);
