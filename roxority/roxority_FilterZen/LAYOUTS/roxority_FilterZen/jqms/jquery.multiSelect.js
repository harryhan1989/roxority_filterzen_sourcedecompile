...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		multiSelect: function(o, callback) {
			...
			$(this).each( function() {
				...
				$(select).find('OPTION').each( function() {
					if( $(this).val() != '' ) {
						html += '<label><input type="checkbox" name="' + $(select).attr('name') + '" value="' + roxSlimAttributeEncode($(this).val()) + '"';
				...
				$(select).next('.multiSelect').next('.multiSelectOptions').find('INPUT:checkbox').click( function() {
					...
					$(this).parent().parent().find('INPUT:checkbox').each(function(i, cb) {
						if ($(cb).attr('checked'))
							checkCount ++;
					});
					if (!checkCount) {
						$(this).parent().parent().parent().find('INPUT:checkbox.selectAll').attr('checked', true).parent().addClass('checked');
						$(this).parent().parent().parent().find('INPUT:checkbox').each(function(i, cb) {
							var $cb = $(cb);
							if ((!$cb.val()) || ($cb.val() == '0478f8f9-fbdc-42f5-99ea-f6e8ec702606'))
								setTimeout((function(theCb) { return function() { theCb.click().attr('checked', true).parent().addClass('checked');; }; })($cb), 125);
						});
					}
					$(this).parent().parent().multiSelectUpdateSelected(o);
				});
				...
				$(select).next('.multiSelect').keydown( function(e) {
					...
					if( $(this).next('.multiSelectOptions').is(':visible') ) {
						...
						if( e.keyCode == 13 || e.keyCode == 32 ) {
							...
							// Other checkboxes
								$(this).next('.multiSelectOptions').find('LABEL.hover INPUT:checkbox').trigger('click');
								if( $(this).next('.multiSelectOptions').find('LABEL.hover INPUT:checkbox').attr('checked') ) {
									// Uncheck
		...
		multiSelectOptionsHide: function() {
			var theID = $(this).next('.multiSelectOptions')[0].parentNode.parentNode.id, partID = 'g_' + $(this).next('.multiSelectOptions')[0].parentNode.parentNode.parentNode.parentNode.parentNode.id.substr('roxfilterinner_g_'.length), pos, oldVals = roxMultiLastVals[theID], vals = [];
			if (($.inArray(theID, roxAutoReposts) >= 0) &&  (oldVals != null)) {
				$($(this).next('.multiSelectOptions')[0].parentNode.parentNode).find('INPUT:checkbox:checked').each(function() {
					if ($.inArray(this.value, vals) < 0)
						vals[vals.length] = this.value;
				});
				if (oldVals.length != vals.length) {
					roxRefreshFilters(partID);
				} else
			...
			$(this).next('.multiSelectOptions').height('auto');
			$(this).next('.multiSelectOptions').css({ width: 'auto' });
			$(this).removeClass('active').next('.multiSelectOptions').hide();
		},

		// Show the dropdown
		multiSelectOptionsShow: function() {
			var vals = [], theID = $(this).next('.multiSelectOptions')[0].parentNode.parentNode.id;
			if ($.inArray(theID, roxAutoReposts) >= 0) {
				$(this).next('.multiSelectOptions').find('INPUT:checkbox:checked').each(function() {
					vals[vals.length] = this.value;
				});
				roxMultiLastVals[theID] = vals;
			}
			...
			// Position it
			var offset = $(this).position(), nuWidth = $(this).width() + 24, maxWidth = Math.abs($(window).width() - 24 - offset.left), minWidth = nuWidth, maxHeight = $(window).height() - 24 - (offset.top + $(this).outerHeight());
			$(this).next('.multiSelectOptions').css({ top:  offset.top + $(this).outerHeight() + 'px' });
			$(this).next('.multiSelectOptions').css({ left: offset.left + 'px' });
			if ($(this).next('.multiSelectOptions').height() > maxHeight)
				$(this).next('.multiSelectOptions').height(maxHeight);
			if (nuWidth > maxWidth)
				nuWidth = maxWidth;
			if (nuWidth < minWidth)
				nuWidth = minWidth;
			$(this).next('.multiSelectOptions').css({ width: nuWidth + 'px' });
			...
			$(this).next('.multiSelectOptions').hover( function() {
				...
				timer = setTimeout(function() { $(multiSelectCurrent).multiSelectOptionsHide(); $(multiSelectCurrent).unbind('hover'); }, 250);
			});
		},
		...
		multiSelectUpdateSelected: function(o) {
			...
			if( i == 0 ) {
				...
				if( o.oneOrMoreSelected == '*' ) {
					...
					$(this).prev('INPUT.multiSelect').val( display.replace(/\u00A0/g, '') );
				} else {
